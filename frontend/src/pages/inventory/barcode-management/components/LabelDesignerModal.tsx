...
// import { useBarcodeCartStore, BarcodeCartItem } from '@/stores/barcodeCartStore';
...
//     }
//   }, [settings.fields.barcodeImage, settings.format, item.barcode]);

//   const scale = 3.2; // px per mm for preview
...
// export default function LabelDesignerModal({ onClose }: Props) {
//   const { items, updateQty } = useBarcodeCartStore();
...
//   const buildLabelHTML = useCallback((item: BarcodeCartItem): string => {
//     const fmt = settings.format === 'per-item' ? 'CODE128' : settings.format;
//     const barcodeId = `bc-${item.itemId}-${Math.random().toString(36).slice(2)}`;

//     const rows: string[] = [];
//     if (settings.fields.companyName) rows.push(`<div style="font-weight:700;font-size:7pt;overflow:hidden;white-space:nowrap;">${mockCompany.name}</div>`);
//     if (settings.fields.productName) rows.push(`<div style="font-weight:600;font-size:8pt;overflow:hidden;white-space:nowrap;">${item.name}</div>`);
//     if (settings.fields.brand) rows.push(`<div style="font-size:6.5pt;color:#555;overflow:hidden;white-space:nowrap;">${item.brand}</div>`);
//     if (settings.fields.category) rows.push(`<div style="font-size:6pt;color:#888;overflow:hidden;white-space:nowrap;">${item.category}</div>`);

//     const priceRow: string[] = [];
//     if (settings.fields.mrp) priceRow.push(`<span style="font-size:7pt;font-weight:600;">MRP: ₹${item.mrp}</span>`);
//     if (settings.fields.saleRate) priceRow.push(`<span style="font-size:7pt;font-weight:700;color:#4f46e5;">₹${item.saleRate}</span>`);
//     if (priceRow.length) rows.push(`<div style="display:flex;gap:8px;margin-top:auto;">${priceRow.join('')}</div>`);

//     if (settings.fields.barcodeImage) {
//       rows.push(`<svg id="${barcodeId}" style="width:100%;max-height:28px;"></svg>`);
//     }
//     if (settings.fields.barcodeNumber) {
//       rows.push(`<div style="text-align:center;font-family:monospace;font-size:6pt;color:#444;">${item.barcode}</div>`);
//     }
//     if (settings.fields.customText && settings.customText) {
//       rows.push(`<div style="font-size:6pt;color:#666;overflow:hidden;white-space:nowrap;">${settings.customText}</div>`);
//     }

//     const initScript = settings.fields.barcodeImage
//       ? `<script>
//           (function() {
...
//                   format: '${fmt === 'EAN13' ? 'EAN13' : 'CODE128'}',
//                   width: 1.2, height: 28, displayValue: false, margin: 0
//                 });
//               } catch(e) {}
//             }
//           })();
...
//     return `
//       <div class="label" style="width:${w}mm;height:${h}mm;box-sizing:border-box;padding:2mm;border:1px solid #000;display:flex;flex-direction:column;overflow:hidden;page-break-inside:avoid;break-inside:avoid;background:white;color:black;">
//         ${rows.join('')}
//         ${initScript}
//       </div>`;
//   }, [settings, w, h]);
...
//       Array.from({ length: item.labelQty }, () => buildLabelHTML(item))
//     ).join('');
...
//       .label { background: white !important; color: black !important; border: 1px solid #000 !important; }
//     }
//   </style>
// </head>
// <body>
...
//             <div className="w-8 h-8 flex items-center justify-center bg-indigo-100 rounded-lg text-indigo-600">
//               <i className="ri-layout-2-line text-lg" />
//             </div>
//             <div>
//               <h2 className="font-semibold text-slate-800 text-base">Label Designer</h2>
//               <p className="text-xs text-slate-400">{items.length} items · {totalLabels} total labels</p>
//             </div>
//           </div>
//           <button
//             onClick={onClose}
...
//                       onChange={(e) => setSettings((p) => ({ ...p, customW: parseInt(e.target.value) || 80 }))}
...
//                       onChange={(e) => setSettings((p) => ({ ...p, customH: parseInt(e.target.value) || 40 }))}
//                       className="w-full border border-slate-200 rounded-md px-2 py-1.5 text-sm outline-none focus:border-indigo-400"
//                     />
//                   </div>
//                 </div>
...
//                 ] as { val: BarcodeFormat; label: string }[]).map(({ val, label }) => (
//                   <button
//                     key={val}
...
//                 ] as { key: keyof LabelSettings['fields']; label: string }[]).map(({ key, label }) => (
//                   <label
//                     key={key}
...
//                   Preview: {w}mm × {h}mm · {settings.format === 'EAN13' ? 'EAN-13' : settings.format === 'CODE128' ? 'Code128' : 'Per Item'}
//                 </p>
//               </div>
//             </div>
...
//                       <th className="text-left px-4 py-2.5 text-xs font-semibold text-slate-500">Item Name</th>
//                       <th className="text-center px-4 py-2.5 text-xs font-semibold text-slate-500">Labels Qty</th>
//                     </tr>
//                   </thead>
...
//                             className="w-16 text-center border border-slate-200 rounded-md py-1 text-sm outline-none focus:border-indigo-400"
//                           />
//                         </td>
//                       </tr>
//                     ))}
...
import { useBarcodeCartStore, BarcodeCartItem } from '@/stores/barcodeCartStore';
import { mockCompany } from '@/mocks/masters';
import { getData, postData } from '@/services/FetchNodeServices'; // your existing service
...
export default function LabelDesignerModal({ onClose }: Props) {
  ...
  // ✅ New: loading/saving state
  const [settingsLoading, setSettingsLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [saveStatus, setSaveStatus] = useState<'idle' | 'saved' | 'error'>('idle');

  const previewItem: BarcodeCartItem = items[0] ?? {
    itemId: 'preview',
    name: 'Sample Product Name',
    brand: 'Brand',
    category: 'Category',
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        const response = await getData('api/label-settings/get-label-settings');
  ...
  const saveSettingsToBackend = useCallback(async (settingsToSave: LabelSettings) => {
    try {
      setSaving(true);
      setSaveStatus('idle');
      const response = await postData('api/label-settings/save-label-settings', {
  ...
  const handlePrint = () => {
    ...
    // ✅ Save to backend instead of localStorage (only if rememberSettings is on)
    if (rememberSettings) {
  ...
  if (settingsLoading) {
    return (
      <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
        <div className="bg-white rounded-xl px-10 py-8 flex flex-col items-center gap-3 shadow-xl">
          <div className="w-8 h-8 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin" />
          <p className="text-sm text-slate-500">Loading label settings...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-xl w-full max-w-6xl max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-80 shrink-0 border-r border-slate-100 overflow-y-auto p-5 space-y-5">
            ...
            {/* ✅ Remember Settings — now saves to backend */}
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={rememberSettings}
            ...
            <button
              onClick={() => saveSettingsToBackend(settings)}
              disabled={saving}
              className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-md border border-indigo-200 text-indigo-600 text-xs font-medium hover:bg-indigo-50 transition-colors disabled:opacity-50 cursor-pointer"
            >
              {saving ? (
                <>
          ...
          {/* RIGHT — Preview + Quantities (unchanged) */}
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Live Preview</p>
              <div className="flex flex-col items-center gap-3">
