...
// // ─── Config ───────────────────────────────────────────────────────────────────
// const BASE_URL =
//   (import.meta.env.VITE_API_URL as string) || 'https://asvapi.digiindiasolutions.com';
...
//   MAIN: { label: 'Main', icon: 'ri-store-3-line', cls: 'bg-indigo-100 text-indigo-600' },
//   TRANSIT: { label: 'Transit', icon: 'ri-truck-line', cls: 'bg-amber-100 text-amber-600' },
//   RETURN: { label: 'Return', icon: 'ri-arrow-go-back-line', cls: 'bg-rose-100 text-rose-600' },
//   COLD: { label: 'Cold', icon: 'ri-temp-cold-line', cls: 'bg-sky-100 text-sky-600' },
// };
...
//   loading, error, navIndex, disabled,
...
//   const handleOpen = () => { if (disabled || loading) return; setOpen(true); setTimeout(() => inputRef.current?.focus(), 50); };
...
//         className={`w-full h-10 px-3 rounded-lg border text-sm text-left flex items-center justify-between bg-white focus:outline-none focus:ring-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer ${error
...
//         </span>
//         <i className={`ri-arrow-down-s-line text-[#94a3b8] transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
//       </button>
...
//                     className={`w-full px-3 py-2 text-sm text-left flex items-center justify-between hover:bg-[#f1f5f9] cursor-pointer transition-colors ${o.id === value ? 'bg-[#eef2ff] text-[#4f46e5] font-medium' : 'text-[#1e293b]'
//                       }`}
//                   >
//                     {o.name}
//                     {o.id === value && <i className="ri-check-line text-[#4f46e5]" />}
//                   </button>
//                 </li>
//               ))
//             )}
...
// // ─── WarehouseSelect ─────────────────────────────────────────────────────────
// // A richer searchable dropdown that shows type badges inline
...
// function WarehouseSelect({ value, onChange, warehouses, loading }: WarehouseSelectProps) {
//   const [query, setQuery] = useState('');
//   const [open, setOpen] = useState(false);
//   const containerRef = useRef<HTMLDivElement>(null);
//   const inputRef = useRef<HTMLInputElement>(null);
...
//   const filtered = query.trim()
//     ? warehouses.filter((w) => w.name.toLowerCase().includes(query.toLowerCase()) || w.type.toLowerCase().includes(query.toLowerCase()))
//     : warehouses;
...
//         onClick={handleOpen}
//         disabled={loading}
//         className="w-full h-10 px-3 rounded-lg border border-[#e2e8f0] text-sm text-left flex items-center gap-2 bg-white hover:border-[#4f46e5] hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-[#4f46e5]/20 transition-colors disabled:opacity-50 cursor-pointer"
//       >
//         {selectedBadge ? (
//           <div className={`w-6 h-6 flex items-center justify-center rounded-md shrink-0 ${selectedBadge.cls}`}>
...
//           </div>
//         )}
//         <span className={`flex-1 truncate font-medium ${selected ? 'text-[#1e293b]' : 'text-[#94a3b8]'}`}>
//           {loading ? 'Loading warehouses…' : selected ? selected.name : 'Select warehouse…'}
//         </span>
//         {selectedBadge && (
//           <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded shrink-0 ${selectedBadge.cls}`}>
//             {selectedBadge.label}
//           </span>
//         )}
//         <i className={`ri-arrow-down-s-line text-[#94a3b8] shrink-0 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
//       </button>
...
//                       className={`w-full px-3 py-2.5 flex items-center gap-2.5 hover:bg-[#f8fafc] cursor-pointer transition-colors ${isSelected ? 'bg-indigo-50' : ''
//                         }`}
//                     >
//                       <div className={`w-7 h-7 flex items-center justify-center rounded-lg shrink-0 ${badge?.cls ?? 'bg-slate-100 text-slate-500'}`}>
...
//   isEditing: boolean;
//   onClose: () => void;
//   /** Must throw on API error so the caller can handle it */
...
//   brand: '', hsnCode: '', taxRate: 18, unitId: '', unitName: '',
//   purchaseRate: 0, saleRate: 0, minStockLevel: 5, articleNo: '', isActive: true,
// };

// // ─── ItemForm ─────────────────────────────────────────────────────────────────
// export default function ItemForm({ open, initialData, isEditing, onClose, onSave }: ItemFormProps) {
...
//   // ── State ──────────────────────────────────────────────────────────────────
//   const [form, setForm] = useState<ItemFormData>({ ...defaultForm, ...initialData, warehouseId: isEditing ? initialData.warehouseId : selectedWarehouseId, });
//   const [errors, setErrors] = useState<Record<string, string>>({});
//   const [apiError, setApiError] = useState('');
//   const [isSaving, setIsSaving] = useState(false);
//   const [autoCode, setAutoCode] = useState(!isEditing);
//   const [duplicateWarning, setDuplicateWarning] = useState<string | null>(null);
//   const [barcodeMsg, setBarcodeMsg] = useState<{ type: 'success' | 'info' | 'error'; text: string } | null>(null);
...
//   // Local warehouse selection (synced to global store on change)
//   const [localWarehouseId, setLocalWarehouseId] = useState(isEditing ? form.warehouseId : selectedWarehouseId ?? '');
...
//           fetch(`${BASE_URL}/api/v1/categories/all`, { headers: authHeaders() }),
...
//       setLocalWarehouseId(selectedWarehouseId ?? '');
//       setTimeout(() => barcodeRef.current?.focus(), 80);
//     }
//   }, [open, isEditing]); // intentionally omit initialData / selectedWarehouseId to avoid re-runs

//   // ── Helpers ────────────────────────────────────────────────────────────────
//   const update = (field: keyof ItemFormData, value: string | number | boolean) => {
...
//     update('unitName', unit?.name ?? '');
//   };

//   const onWarehouseChange = (id: string, name: string) => {
//     setLocalWarehouseId(id);
//     setSelectedWarehouse(id, name); // write to global store
//     setForm((prev) => ({
//       ...prev, warehouseId: id,
//     }));
//   };
...
//           if (!isSelf) {
//             setDuplicateWarning(`Barcode already used by: "${found.name}" (Code: ${found.code})`);
//           } else {
//             setBarcodeMsg({ type: 'success', text: `Item found: ${mapped.name}` });
...
//           const isSelf = isEditing && found.code === initialData?.code;
//           if (!isSelf) {
//             setDuplicateWarning(`An item named "${found.name}" already exists (Code: ${found.code})`);
//           } else {
//             setBarcodeMsg({ type: 'success', text: `Item found: ${mapped.name}` });
//             setTimeout(() => setBarcodeMsg(null), 3000);
//           }
...
//     if (!autoCode && !form.code.trim()) e.code = 'Item code is required';
//     if (!form.hsnCode.trim()) e.hsnCode = 'HSN code is required';
//     if (!localWarehouseId) e.warehouse = 'Please select a warehouse';
//     if (
//       form.saleRate > 0 && form.purchaseRate > 0 &&
//       form.saleRate < form.purchaseRate
//     ) {
...
//       ? (((form.saleRate - form.purchaseRate) / form.purchaseRate) * 100).toFixed(1)
//       : null;
...
//             </h3>
//             <p className="text-xs text-[#64748b] mt-0.5">Scan barcode or fill details manually</p>
//           </div>
//           <button
//             onClick={onClose}
...
//                 Details have been auto-filled. Clear the name/barcode to create a new item.
//               </p>
//             </div>
//           </div>
//         )}
...
//                 <i className="ri-error-warning-line" /> {errors.warehouse}
//               </p>
//             )}
//           </div>
...
//             </div>
//             {barcodeMsg && (
//               <p className={`text-xs flex items-center gap-1 ${barcodeMsg.type === 'success' ? 'text-green-600'
//                 : barcodeMsg.type === 'error' ? 'text-red-500'
//                   : 'text-amber-600'
//                 }`}>
//                 <i className={
//                   barcodeMsg.type === 'success' ? 'ri-checkbox-circle-fill'
//                     : barcodeMsg.type === 'error' ? 'ri-error-warning-line'
//                       : 'ri-information-line'
//                 } />
//                 {barcodeMsg.text}
//               </p>
...
//               data-nav-index={1}
//               className={`w-full h-10 px-3 rounded-lg border text-sm text-[#1e293b] bg-white focus:outline-none focus:ring-2 transition-colors ${errors.name
//                 ? 'border-red-400 focus:ring-red-200'
//                 : 'border-[#e2e8f0] focus:border-[#4f46e5] focus:ring-[#4f46e5]/20'
//                 }`}
//             />
...
//               data-nav-index={2}
//               className={`w-full h-10 px-3 rounded-lg border text-sm font-mono focus:outline-none focus:ring-2 transition-colors ${autoCode
//                 ? 'bg-[#f8fafc] text-slate-400 cursor-default border-[#e2e8f0]'
...
//             <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wide flex items-center gap-2">
//               Category
//               {dropdownsLoading && <i className="ri-loader-4-line animate-spin text-[#94a3b8]" />}
//             </label>
//             <SearchableSelect
...
//           {/* ── BRAND ── */}
//           <div className="space-y-1.5">
//             <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wide">Brand</label>
//             <input
//               type="text"
//               value={form.brand}
...
//               data-nav-index={5}
//               className={`w-full h-10 px-3 rounded-lg border text-sm font-mono text-[#1e293b] bg-white focus:outline-none focus:ring-2 transition-colors ${errors.hsnCode
//                 ? 'border-red-400 focus:ring-red-200'
//                 : 'border-[#e2e8f0] focus:border-[#4f46e5] focus:ring-[#4f46e5]/20'
//                 }`}
//             />
...
//           {/* ── ARTICLE NO ── */}
//           <div className="space-y-1.5">
//             <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wide">Article No.</label>
//             <input
//               type="text"
...
//             <div className="space-y-1.5">
//               <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wide">Purchase Rate (₹)</label>
//               <input
//                 type="text"
//                 inputMode="numeric"
//                 value={form.purchaseRate || ''}
//                 onChange={(e) => update('purchaseRate', parseFloat(e.target.value) || 0)}
//                 placeholder="0"
//                 data-nav-index={7}
//                 className="w-full h-10 px-3 rounded-lg border border-[#e2e8f0] text-sm text-[#1e293b] bg-white focus:outline-none focus:border-[#4f46e5] focus:ring-2 focus:ring-[#4f46e5]/20"
//               />
//             </div>
//             <div className="space-y-1.5">
//               <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wide">Sale Rate (₹)</label>
//               <input
//                 type="text"
//                 inputMode="numeric"
//                 value={form.saleRate || ''}
//                 onChange={(e) => update('saleRate', parseFloat(e.target.value) || 0)}
//                 placeholder="0"
//                 data-nav-index={8}
//                 className={`w-full h-10 px-3 rounded-lg border text-sm text-[#1e293b] bg-white focus:outline-none focus:ring-2 transition-colors ${errors.saleRate
//                   ? 'border-red-400 focus:ring-red-200'
//                   : 'border-[#e2e8f0] focus:border-[#4f46e5] focus:ring-[#4f46e5]/20'
//                   }`}
//               />
//               {errors.saleRate && <p className="text-xs text-red-500">{errors.saleRate}</p>}
//             </div>
//           </div>

//           {/* Margin indicator */}
//           {marginVal !== null && (
//             <div className={`flex items-center gap-2 text-xs font-medium px-3 py-2 rounded-lg ${marginVal < 0 ? 'bg-red-50 text-red-600'
//               : marginVal < 10 ? 'bg-amber-50 text-amber-600'
//                 : 'bg-green-50 text-green-600'
//               }`}>
//               <i className={marginVal < 0 ? 'ri-arrow-down-line' : 'ri-arrow-up-line'} />
//               Margin: {margin}%&nbsp;&nbsp;|&nbsp;&nbsp;Profit: ₹{(form.saleRate - form.purchaseRate).toLocaleString()} per unit
//             </div>
//           )}
...
//                   e.preventDefault();
//                   update('taxRate', GST_RATES[Math.min(idx + 1, GST_RATES.length - 1)]);
//                 }
...
//                   className={`flex-1 h-10 rounded-lg text-xs font-bold border transition-colors cursor-pointer ${form.taxRate === r
...
//               ))}
//             </div>
//             <p className="text-xs text-[#94a3b8]">Use ← → arrow keys to change when focused</p>
//           </div>
...
//           {/* ── MIN STOCK ── */}
//           <div className="space-y-1.5">
//             <label className="text-xs font-semibold text-[#64748b] uppercase tracking-wide">Min Stock Level</label>
//             <input
//               type="text"
//               inputMode="numeric"
...
//                 update('minStockLevel', val === '' ? 0 : parseInt(val));
//               }}
//               data-nav-index={11}
//               className="w-full h-10 px-3 rounded-lg border border-[#e2e8f0] text-sm text-[#1e293b] bg-white focus:outline-none focus:border-[#4f46e5] focus:ring-2 focus:ring-[#4f46e5]/20"
//             />
...
//                 }`}
//             >
//               {/* ✅ FIX: correct translate direction */}
...
//           {localWarehouseId && warehouses.length > 0 && (() => {
//             const wh = warehouses.find((w) => w.id === localWarehouseId);
//             const badge = wh ? (TYPE_BADGE[wh.type] ?? null) : null;
//             return wh ? (
//               <div className={`flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-md ${badge?.cls ?? 'bg-slate-100 text-slate-500'}`}>
//                 <i className={`${badge?.icon ?? 'ri-store-3-line'} text-xs`} />
...
//                 : <><i className="ri-save-line" /> {isEditing ? 'Update Item' : 'Save Item'}</>
//               }
//             </button>
//           </div>
//         </div>
...
// FIX: defaultForm now includes warehouseId so ItemFormData is fully satisfied
const defaultForm: ItemFormData = {
  name: '',
  code: '',
  barcode: '',
...
export default function ItemForm({
  ...
  const { selectedWarehouseId, setSelectedWarehouse } = useWarehouseStore();
  const { hasControl } = useAuth();
  // FIX: stable value captured once — used in dep array safely
  const canViewAll = hasControl('viewAllWarehouses');
  ...
  const [form, setForm] = useState<ItemFormData>(() => ({
    ...defaultForm,
    ...initialData,
    // FIX: safe optional chaining; edit mode uses item's own warehouse, add mode uses global store
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // silent — dropdowns stay empty; user can retry by reopening
      } finally {
  ...
  // ── Fetch warehouses ───────────────────────────────────────────────────────
  // FIX: canViewAll added to dep array so stale closure is impossible
  useEffect(() => {
    let mounted = true;
    setWarehousesLoading(true);
    void (async () => {
  ...
  // ── Reset on open ──────────────────────────────────────────────────────────
  // FIX: edit mode preserves the item's warehouseId; add mode uses global store
  useEffect(() => {
    if (!open) return;
  ...
  // FIX: using functional setState so update() itself never needs to be a dep
  ...
  // ── Validation ─────────────────────────────────────────────────────────────
  // FIX: all validation reads from `form` directly — single source of truth
  ...
  const handleSave = async () => {
    // Read latest form state before validating
    let snapshot!: ItemFormData;
    setForm((f) => {
      snapshot = f;
      return f;
    });
    // Allow state flush then validate
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {marginVal !== null && (
            ...
            >
              ...
              {(form.saleRate - form.purchaseRate).toLocaleString()} per unit
            </div>
          )}
