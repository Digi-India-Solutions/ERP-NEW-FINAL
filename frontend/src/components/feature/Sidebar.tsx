  ...
  items: [
    ...
    },
    {
      ...
      items: [
        {
          label: 'Incoming QC',
          path: '/manufacturing/quality/incoming-qc',
          icon: 'ri-shield-check-line',
        },
        {
          label: 'In-Process QC',
          path: '/manufacturing/quality/in-process-qc',
          icon: 'ri-shield-check-line',
        },
        {
          label: 'Final QC',
          path: '/manufacturing/quality/final-qc',
          icon: 'ri-award-line',
        },
        {
          label: 'NCR / CAPA',
          path: '/manufacturing/quality/ncr-capa',
...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const visibleMastersItems = mastersGroup.items.filter((item) => {
    if (item.path === '/masters/warehouses')
      return hasPermission(MODULES.WAREHOUSES, 'view');
    if (item.path === '/masters/parties')
      return hasPermission(MODULES.PARTIES, 'view');
    if (item.path === '/masters/items')
      return hasPermission(MODULES.ITEMS, 'view');
    if (item.path === '/masters/categories')
      return hasPermission(MODULES.CATEGORIES, 'view');
    if (item.path === '/masters/work-centers')
      return hasPermission(MODULES.WORK_CENTERS, 'view');
    if (item.path === '/masters/company')
      return hasPermission(MODULES.COMPANY, 'view');
    if (item.path === '/masters/machines')
      return hasPermission(MODULES.MACHINE, 'view');
    if (item.path === '/masters/shifts')
      return hasPermission(MODULES.SHIFT, 'view');
    if (item.path === '/masters/operators')
      return hasPermission(MODULES.OPERATOR, 'view');
    if (item.path === '/masters/cost-centers')
      return hasPermission(MODULES.COST_CENTRE, 'view');
    if (item.path === '/masters/rejection-codes')
      return hasPermission(MODULES.REJECTION_CODE, 'view');
    if (item.path === '/masters/downtime-codes')
      return hasPermission(MODULES.DOWNTIME_CODE, 'view');
    if (item.path === '/masters/quality-parameters')
      return hasPermission(MODULES.QUALITY_PARAMETER, 'view');
    if (item.path === '/masters/inspection-checklists')
      return hasPermission(MODULES.INSPECTION_CHECKLIST, 'view');
    if (item.path === '/masters/routing')
      return hasPermission(MODULES.ROUTING, 'view');
    if (item.path === '/masters/routing/new')
      return hasPermission(MODULES.ROUTING, 'create');
  ...
  const visibleSalesItems = salesGroup.items.filter((item) => {
    if (item.path === '/sales/invoices')
      return hasPermission(MODULES.SALES_INVOICE, 'view');
    if (item.path === '/sales/payments')
      return hasPermission(MODULES.SALES_PAYMENT, 'view');
    if (item.path === '/sales/returns')
      return hasPermission(MODULES.SALE_RETURN, 'view');
    if (item.path === '/sales/challans')
      return hasPermission(MODULES.CHALLAN, 'view');
    return false;
  });

  const visiblePurchaseItems = purchaseGroup.items.filter((item) => {
    if (item.path === '/purchase/orders')
      return hasPermission(MODULES.PURCHASE_ORDER, 'view');
    if (item.path === '/purchase/invoices')
      return hasPermission(MODULES.PURCHASE_INVOICE, 'view');
    if (item.path === '/purchase/payments')
      return hasPermission(MODULES.PURCHASE_PAYMENT, 'view');
    if (item.path === '/purchase/grn')
      return hasPermission(MODULES.GRN_HISTORY, 'view');
    if (item.path === '/purchase/returns')
      return hasPermission(MODULES.PURCHASE_RETURN, 'view');
    return false;
  });
  ...
  const visibleInventoryItems = inventoryGroup.items.filter((item) => {
    ...
    if (item.path === '/inventory/receiving')
      return hasPermission(MODULES.STOCK_RECEIVING, 'create');
    if (item.path === '/inventory/stock-entries')
      return hasPermission(MODULES.STOCK_ENTRIES, 'view');
    if (item.path === '/inventory/transfer')
      return hasPermission(MODULES.STOCK_TRANSFER, 'view');
    if (item.path === '/inventory/adjustment')
      return (
        hasPermission(MODULES.STOCK_ADJUSTMENT, 'view') ||
        hasControl('approveStockAdjustment')
      );
    if (item.path === '/inventory/gate-pass/outward')
      return hasPermission(MODULES.GATE_PASS_OUTWARD, 'view');
    if (item.path === '/inventory/gate-pass/inward')
      return hasPermission(MODULES.GATE_PASS_INWARD, 'view');
    return false;
  });
  ...
  const visibleReportsItems = reportsGroup.items.filter((item) => {
    const fin = hasControl('viewFinancialReports');
    if (item.path === '/reports/stock-summary')
      return hasPermission(MODULES.REPORT_STOCK_SUMMARY, 'view') || fin;
    if (item.path === '/reports/stock-ledger')
      return hasPermission(MODULES.REPORT_STOCK_LEDGER, 'view') || fin;
    if (item.path === '/reports/low-stock')
      return hasPermission(MODULES.REPORT_LOW_STOCK, 'view') || fin;
    if (item.path === '/reports/purchase-register')
      return hasPermission(MODULES.REPORT_PURCHASE_REG, 'view') || fin;
    if (item.path === '/reports/gst-purchase')
      return hasPermission(MODULES.REPORT_GST_PURCHASE, 'view') || fin;
    if (item.path === '/reports/sales-register')
      return hasPermission(MODULES.REPORT_SALES_REG, 'view') || fin;
    if (item.path === '/reports/gst-sales')
      return hasPermission(MODULES.REPORT_GST_SALES, 'view') || fin;
    if (item.path === '/reports/outstanding')
      return hasPermission(MODULES.REPORT_OUTSTANDING, 'view') || fin;
    if (item.path === '/reports/day-book')
      return hasPermission(MODULES.REPORT_DAY_BOOK, 'view') || fin;
    if (item.path === '/reports/party-ledger')
      return hasPermission(MODULES.REPORT_PARTY_LEDGER, 'view') || fin;
    if (item.path === '/reports/manufacturing')
      return hasPermission(MODULES.REPORT_MANUFACTURING, 'view') || fin;
    if (item.path === '/reports/finance')
      return hasPermission(MODULES.REPORT_FINANCE, 'view') || fin;
    if (item.path === '/reports/gst')
      return hasPermission(MODULES.REPORT_GST, 'view') || fin;
    if (item.path === '/reports/dashboard')
      return hasPermission(MODULES.REPORT_DASHBOARD, 'view') || fin;
    return false;
  });

  const visiblePrintItems = printGroup.items.filter((item) => {
    if (item.path === '/print/barcode-management')
      return hasPermission(MODULES.BARCODE_PRINT, 'view');
    return false;
  });
  ...
  const canSeeUsers =
    hasPermission(MODULES.USERS, 'view') || hasControl('manageUserPermissions');
  const canSeeSettings =
    hasPermission(MODULES.SETTINGS, 'view') ||
    hasControl('manageUserPermissions');
  const canSeePrint = visiblePrintItems.length > 0;
  const canSeeReports =
    visibleReportsItems.length > 0 || hasControl('viewFinancialReports');
  ...
  const visiblePurchaseGroup = {
    ...purchaseGroup,
    items: visiblePurchaseItems,
  };
  const visibleInventoryGroup = {
    ...inventoryGroup,
  ...
  const BrandHeader = ({ guardMode = false }: { guardMode?: boolean }) => (
    <div className="flex items-center h-[60px] px-4 border-b border-white/10 shrink-0">
      ...
      >
        <i
          className={`${collapsed ? 'ri-menu-unfold-line' : 'ri-menu-fold-line'} text-base`}
        />
      </button>
    </div>
  );
  ...
  const asideClass =
    'flex flex-col h-screen bg-[#1e293b] text-white transition-all duration-300 ease-in-out shrink-0';
